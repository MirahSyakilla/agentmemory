...
// Regression: repeated session-stop notifications can arrive from multiple
// hosts or stale-session recovery. consolidate-pipeline + auto-crystallize are
// full-corpus LLM work with no internal "nothing changed" guard, so the debounce
// bounds corpus consolidation to once per cooldown.
