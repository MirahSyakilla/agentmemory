...
// OpenCode context injection follows the server-side opt-in flag and reuses
// the /session/start context when enabled.
describe("OpenCode plugin auto-context injection (#431)", () => {
  ...
  it("honors the server-side context injection flag", () => {
    expect(plugin).toMatch(/getJson\(["']\/config\/flags["']\)/);
    expect(plugin).toMatch(/key === ["']AGENTMEMORY_INJECT_CONTEXT["']/);
    expect(plugin).toMatch(/const injectContext = await contextInjectionEnabled\(\)/);

    const systemBlock = plugin.slice(
      plugin.indexOf('"experimental.chat.system.transform"'),
      plugin.indexOf('"experimental.session.compacting"'),
    );
    expect(systemBlock).toMatch(/if \(!injectContext\) return/);

    const compactionBlock = plugin.slice(plugin.indexOf('"experimental.session.compacting"'));
    expect(compactionBlock).toMatch(/if \(!injectContext\) return/);
  });
