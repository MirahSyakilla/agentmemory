...
<relationship source="src/index.ts" target="main" type="uses" weight="0.9"/>
  ...
  it("compact graph queries omit high-cardinality provenance arrays", async () => {
    const node: GraphNode = {
      id: "compact-node",
      type: "concept",
      name: "compact",
      properties: {},
      sourceObservationIds: Array.from({ length: 1000 }, (_, i) => `obs_${i}`),
      sourceSessionIds: Array.from({ length: 1000 }, (_, i) => `ses_${i}`),
    ...
    expect(result.nodes[0].sourceObservationIds).toEqual([]);
    expect(result.nodes[0].sourceSessionIds).toEqual([]);
    expect(result.edges[0].sourceObservationIds).toEqual([]);
  });
