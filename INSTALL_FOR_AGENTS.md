...
Expect: the agent now lists agentmemory's tools. With the server running you should see the full set of 74 tools (for example `memory_retrieval_plan`, `memory_save`, `memory_smart_search`, `memory_sessions`). If you see only 7 tools, the MCP shim could not reach a server, see Troubleshooting.
