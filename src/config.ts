    ...
    process.stderr.write(
      pc.dim(
        "[agentmemory] No LLM provider key set — running zero-LLM (BM25 + on-device embeddings). " +
          "Set ANTHROPIC_API_KEY (or OPENAI_SUMMARIZE_API_KEY/GEMINI/OPENROUTER/MINIMAX) in ~/.agentmemory/.env for LLM compression and summaries. " +
...
const SESSION_IDLE_TIMEOUT_DEFAULT_MS = 24 * 60 * 60 * 1000;
const SESSION_IDLE_SWEEP_INTERVAL_DEFAULT_MS = 60 * 60 * 1000;

export function getSessionIdleTimeoutMs(): number {
  return Math.max(
    0,
    safeParseInt(
      getMergedEnv()["AGENTMEMORY_SESSION_IDLE_TIMEOUT_MS"],
      SESSION_IDLE_TIMEOUT_DEFAULT_MS,
    ),
  );
}

export function getSessionIdleSweepIntervalMs(): number {
  return Math.max(
    60_000,
    safeParseInt(
      getMergedEnv()["AGENTMEMORY_SESSION_IDLE_SWEEP_INTERVAL_MS"],
      SESSION_IDLE_SWEEP_INTERVAL_DEFAULT_MS,
    ),
  );
}
