...
export function registerEventTriggers(sdk: ISdk, kv: StateKV): void {
  ...
  sdk.registerFunction("event::session::stopped", async (data: { sessionId: string; skipConsolidation?: boolean }) => {
    ...
    // Crystals + lessons consolidation. The stopped lifecycle is the single
    // source of truth: the terminal session-end hook and stale-session
    // recovery both enter through event::session::stopped.
    ...
    //
    // Debounce: multiple hosts can still report the same session stop or
    // recover several stale sessions together. consolidate-pipeline +
    // auto-crystallize are full-corpus LLM work with no internal "nothing
    // changed" guard, so bound them to one run per cooldown window.
